...
# 定义一个包含所有迭代次数的数组
itratioin=(10 142 75 154 150 143 234 225 367 154 237)
...
for t in 2; do
    ...
    if [ $index -ge 0 ] && [ $index -lt ${#itratioin[@]} ]; then
        ...
        python train_continual.py --dist-url auto --num-gpus 1 --config-file configs/ade20k/panoptic-segmentation/100-5.yaml \
            CONT.TASK ${t} CONT.WEIGHTED_SAMPLE False SOLVER.BASE_LR 0.0 SOLVER.MAX_ITER $iter CONT.COLLECT_QUERY_MODE True OUTPUT_DIR ./output/ps/test/step${t} \
        ...
        python train_continual.py --dist-url auto --num-gpus 4 --config-file configs/ade20k/panoptic-segmentation/100-5.yaml \
            CONT.TASK ${t} CONT.WEIGHTED_SAMPLE False SOLVER.BASE_LR 0.00005 SOLVER.MAX_ITER 50 OUTPUT_DIR ./output/ps/test/step${t} \
