...
python train_continual.py --resume --num-gpus 8 --dist-url auto --config-file configs/ade20k/panoptic-segmentation/100-10_inc.yaml \
  CONT.TASK 2 SOLVER.BASE_LR 0.00005 SOLVER.MAX_ITER 10000 MODEL.MASK_FORMER.USE_TEXT_EMBEDDING False OUTPUT_DIR ./output/ps/100-10/step2 \
  CONT.WEIGHTS output/ps/100-10/step1/model_final.pth
for t in 3 4 5 6; do
  python train_continual.py --num-gpus 8 --dist-url auto --config-file configs/ade20k/panoptic-segmentation/100-10_inc.yaml \
  CONT.TASK ${t} SOLVER.BASE_LR 0.00005 SOLVER.MAX_ITER 10000 MODEL.MASK_FORMER.USE_TEXT_EMBEDDING False OUTPUT_DIR ./output/ps/100-10/step${t}
done
# for t in 2; do
#   python train_continual.py --num-gpus 8 --dist-url auto --config-file configs/ade20k/panoptic-segmentation/100-10_inc.yaml \
