...
for t in 9 10 11; do
    ...
    if [ $index -ge 0 ] && [ $index -lt ${#itratioin[@]} ]; then
        ...
        python train_continual.py --dist-url auto --num-gpus 1 --config-file configs/ade20k/panoptic-segmentation/100-5.yaml \
            CONT.TASK ${t} SOLVER.BASE_LR 0.0 SOLVER.MAX_ITER $iter CONT.COLLECT_QUERY_MODE True CONT.CUMULATIVE_PSDNUM True OUTPUT_DIR ./output/ps/100-5_passSelfCross_fakequery_psdWeight_cum/step${t} \
        ...
        python train_continual.py --dist-url auto --num-gpus 4 --config-file configs/ade20k/panoptic-segmentation/100-5.yaml \
            CONT.TASK ${t} SOLVER.BASE_LR 0.00005 SOLVER.MAX_ITER 5000 CONT.COLLECT_QUERY_MODE False OUTPUT_DIR ./output/ps/100-5_passSelfCross_fakequery_psdWeight_cum/step${t} \
